...
start([InputFile, OutputFile]) ->
    ...
    {Is_negative_cycle_present, D} = bellman_ford(P, V, E, G, S, X),
    if
        Is_negative_cycle_present == true ->
            io:format("[ERROR] The input graph contains a -ve cycle~n");
        Is_negative_cycle_present == false ->
...
create_graph(IFH, E, G) ->
    ...
    create_graph(IFH, E-1, G ++ [{list_to_integer(Us), list_to_integer(Vs), list_to_integer(Ws)}]).
...
bellman_ford(P, V, E, G, S, D) ->
    ...
    D1 = setelement(S,D,0),
    D2 = relax(1, V, E, G, D1),
    Status = check_negative_cycle(1, E, G, D2),
    {Status, D2}.
...
% Check for Ny negative weight cycles in the graph
check_negative_cycle(X, E, G, D) ->
    if
        X =< E ->
            Dist_U = element(element(1,element(X,G)),D),
            Dist_V = element(element(2,element(X,G)),D),
            W = element(3,element(X,G)),
            if
                (Dist_U =/= inf) and (Dist_U + W < Dist_V) -> true;
                true -> check_negative_cycle(X+1, E, G, D)
            end;
        X > E -> false
    end.
